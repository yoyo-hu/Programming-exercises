...
void reverse(StaticLinkedList* Alist, struct memoryManager* Amanager) {

	if (Alist->size == 0 || Alist->size == 1)
		return;
	int front = Alist->head;
	int back = Amanager->Memory[front].next;
	int temp= Amanager->Memory[back].next;
	Amanager->Memory[front].next = -1;
	for (int i = 0; i < Alist->size-1; i++) {
		Amanager->Memory[back].next = front;
		if (i == Alist->size - 1 - 1)
			break;
		front = back;
		back = temp;
