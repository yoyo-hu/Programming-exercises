...
int main() {
	...
	scanf("%s", sub);
	for (int i = 0; i < 35; i++) {
		if (strcmp(student[i].first_name, sub) == 0) {
			print(i);
		}
		else if (strcmp(student[i].last_name, sub) == 0){
			print(i);
		}
	}
	return 0;
...
void print(int x) {
	printf("%d\t", (int)student[x].student_id);
	printf("%s\t", student[x].last_name);
	printf("%s\t", student[x].first_name);
	printf("%d\t", (int)student[x].age);
	printf("%s\t", student[x].sex);
	printf("%d\t",(int) student[x].major);
	printf("%d\t", (int)student[x].advisor);
	printf("%s\n", student[x].city_code);
}